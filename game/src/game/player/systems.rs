    ...
    if current_state.0 == MovementState::AirDash {
      for output in controllers_output.iter() {
        ...
        if player_movement > 0.0 {
          direction +=
            Vec2::new(air_dash.timer.elapsed_secs(), -GRAVITY_STRENGTH / AIR_DASH_SPEED) *
            AIR_DASH_SPEED;
        }
        if player_movement < 0.0 {
          direction +=
  ...
  for output in controllers.iter() {
    if
      ...
      f32::trunc(output.desired_translation[1] * 100.0) / 100.0 -
        f32::trunc(output.effective_translation[1] * 100.0) / 100.0 > 0.1 &&
    ...
    if
      ...
      air_dash.used == false &&
      (current_state.0 == MovementState::Jumping || current_state.0 == MovementState::Falling) &&
